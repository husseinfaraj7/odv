...
# Deployment Validation Script with Supabase
# This script deploys the application to staging and runs validation tests
...
run_local_tests() {
    ...
    # Clean and run tests
    if ./mvnw clean test -Dtest=SupabaseDeploymentValidationTest -q; then
        print_success "Local validation tests passed"
    else
        print_error "Local validation tests failed"
        print_error "Fix Supabase configuration issues before deploying to staging"
        exit 1
    fi
    cd ..
}
...
run_staging_tests() {
    ...
    if ./mvnw test -Dtest=SupabaseDeploymentValidationTest -Dserver.url="$STAGING_URL" -Dspring.profiles.active=staging -q; then
        print_success "Staging validation tests passed"
    else
        print_error "Staging validation tests failed"
        print_error "Review staging environment and Supabase configuration"
        return 1
    fi
    cd ..
}
...
run_production_tests() {
    ...
    if ./mvnw test -Dtest=SupabaseDeploymentValidationTest -Dserver.url="$PRODUCTION_URL" -Dspring.profiles.active=prod -q; then
        print_success "Production validation tests passed"
    else
        print_error "Production validation tests failed"
        print_error "CRITICAL: Production deployment with Supabase validation failed!"
        return 1
    fi
    cd ..
}
