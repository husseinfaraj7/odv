...
public class DatabaseValidationConfig implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        String encodedUrl = validateAndEncodeCredentials();
        String validatedUrl = validateDatabaseUrl(encodedUrl);
    ...
    public String validateAndEncodeCredentials() throws Exception {
        logger.info("Validating and encoding DATABASE_URL credentials...");
        ...
        String originalUrl = databaseUrl.trim();
        logger.info("Original DATABASE_URL: {}", maskCredentials(originalUrl));

        // Detect and encode unencoded special characters in password
        String encodedUrl = detectAndEncodeSpecialCharacters(originalUrl);
        
        if (!originalUrl.equals(encodedUrl)) {
            logger.info("Special characters detected and encoded in DATABASE_URL");
            logger.debug("Encoded DATABASE_URL: {}", maskCredentials(encodedUrl));
        } else {
            logger.info("No unencoded special characters detected in DATABASE_URL");
        }

        return encodedUrl;
    }

    private String detectAndEncodeSpecialCharacters(String databaseUrl) {
        // Define special characters that need URL encoding
        char[] specialChars = {'@', '#', '%', '&', '+', '=', '?', '/', ':'};
        
        // Only process standard format URLs (postgres:// or postgresql://) with credentials in userinfo
        if (!databaseUrl.startsWith("postgres://") && !databaseUrl.startsWith("postgresql://")) {
            return databaseUrl; // Return as-is for JDBC URLs
        }
        ...
        try {
            ...
            if (userInfo == null || !userInfo.contains(":")) {
                return databaseUrl; // No credentials to encode
            }

            String[] credentials = userInfo.split(":", 2);
            if (credentials.length != 2) {
                return databaseUrl; // Invalid format, return as-is
            }
            ...
            for (char specialChar : specialChars) {
                if (password.indexOf(specialChar) != -1) {
                    // Check if it's already properly encoded (e.g., %40 for @)
                    String encodedChar = String.format("%%%02X", (int) specialChar);
                    if (!password.contains(encodedChar)) {
                        needsEncoding = true;
                        break;
                    }
                }
            ...
            if (!needsEncoding) {
                return databaseUrl; // No encoding needed
            }

            // Encode the password using DatabaseConfig utility
            String encodedPassword = DatabaseConfig.urlEncode(password);
            
            // Reconstruct the URL with encoded password
            String encodedUserInfo = username + ":" + encodedPassword;
            ...
            encodedUrl.append(scheme).append("://");
            encodedUrl.append(encodedUserInfo).append("@");
            ...
            logger.info("Successfully encoded special characters in DATABASE_URL password");
            return encodedUrl.toString();

        } catch (URISyntaxException e) {
            logger.warn("Failed to parse DATABASE_URL for credential encoding, returning original URL: {}", e.getMessage());
            return databaseUrl;
        } catch (Exception e) {
            logger.warn("Unexpected error during credential encoding, returning original URL: {}", e.getMessage());
            return databaseUrl;
        }
    }
    ...
    private String convertStandardToJdbcUrl(String standardUrl) {
        ...
        String password;
        try {
            username = DatabaseConfig.urlDecode(credentials[0]);
            password = DatabaseConfig.urlDecode(credentials[1]);
        ...
        try {
            encodedUsername = DatabaseConfig.urlEncode(username);
            encodedPassword = DatabaseConfig.urlEncode(password);
        } catch (RuntimeException e) {
            throw new IllegalArgumentException(
    ...
    private void handleEncodingError(String databaseUrl, URISyntaxException e, String operation, int errorIndex) {
        ...
        String detailedMessage = String.format(
            ...
            "2. URL-encode them using the mappings above or use DatabaseConfig.urlEncode()\n" +
