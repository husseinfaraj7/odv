...
public class BrevoConfig {
    ...
    @Value("${brevo.api.timeout:30}")
    private int timeoutSeconds;

    @Value("${brevo.api.retry.max-attempts:3}")
    private int maxRetryAttempts;

    @Value("${brevo.api.retry.delay-seconds:2}")
    private int retryDelaySeconds;

    @Value("${brevo.api.connection-pool.max-total:20}")
    private int connectionPoolMaxTotal;

    @Value("${brevo.api.connection-pool.max-per-route:10}")
    ...
    public ApiClient brevoApiClient() {
        ...
        apiClient.setConnectTimeout((int) Duration.ofSeconds(timeoutSeconds).toMillis());
        apiClient.setReadTimeout((int) Duration.ofSeconds(timeoutSeconds).toMillis());
        apiClient.setWriteTimeout((int) Duration.ofSeconds(timeoutSeconds).toMillis());
