...
public class OrderController {
    ...
    public ResponseEntity<ApiResponse<String>> updateOrderStatus(
        ...
        try {
            ...
            logger.warn("Invalid status update for order {}: {}", orderNumber, e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(ApiResponse.error("Stato non valido", e.getMessage()));
        } catch (Exception e) {
            logger.error("Error updating order status for order: {}", orderNumber, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(ApiResponse.error("Errore interno del server", "Errore nell'aggiornamento dello stato."));
        }
    }
